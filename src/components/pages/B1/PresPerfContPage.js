...
const PresPerfContPage = () => {
  console.log('NEW PRES PERF CONT PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'pres perf cont', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Pres Perf Cont question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Pres Perf Cont lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Pres Perf Cont - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render pres perf cont, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Pres Perf Cont - Page {currentPage + 1} Progress</h3>
            <p>
