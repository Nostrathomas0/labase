...
const PastPerfContPage = () => {
  console.log('NEW PAST PERF CONT PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'past perf cont', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Past Perf Cont question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Past Perf Cont lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Past Perf Cont - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render Past Perf Cont, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Past Perf Cont - Page {currentPage + 1} Progress</h3>
            <p>
