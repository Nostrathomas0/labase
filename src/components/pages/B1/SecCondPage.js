...
const SecCondPage = () => {
  console.log('NEW SEC COND PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'sec cond', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Sec Cond question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Sec Cond lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Sec Cond - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render sec cond, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Sec Cond - Page {currentPage + 1} Progress</h3>
            <p>
