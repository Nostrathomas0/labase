...
const ModalVerbsPage = () => {
  console.log('NEW MODAL VERBS PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'modal verbs', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Modal Verbs question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Modal Verbs lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Modal Verbs - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render modal verbs, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Modal Verbs - Page {currentPage + 1} Progress</h3>
            <p>
