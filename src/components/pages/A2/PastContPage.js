...
const PastContPage = () => {
  console.log('NEW PAST CONTINUOUS PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A2', 'pastcont', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Past Continuous question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Past Continuous lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Past Continuous - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render pastcont, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Past Continuous - Page {currentPage + 1} Progress</h3>
            <p>
