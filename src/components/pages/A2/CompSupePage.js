//CompSupePage.js - Updated to use MainLayout and ContentParser
...
const CompSupePage = () => {
  console.log('NEW COMP SUPE PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'comp supe', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Comp Supe question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Comp Supe lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Comp Supe - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render Comp Supe, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Comp Supe - Page {currentPage + 1} Progress</h3>
            <p>
