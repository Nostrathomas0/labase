...
const FuturePerfPage = () => {
  console.log('NEW FUTURE PERF PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'future perf', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Future Perf question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Future Perf lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Future Perf - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render future perf, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Future Perfect - Page {currentPage + 1} Progress</h3>
            <p>
