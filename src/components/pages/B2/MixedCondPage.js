...
const MixedCondPage = () => {
  console.log('NEW MIXED COND PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'mixed cond', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Mixed Cond question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Mixed Cond lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Mixed Cond - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render mixed cond, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Mixed Cond - Page {currentPage + 1} Progress</h3>
            <p>
