//VerbsPage.js - Updated to use MainLayout and ContentParser
import React, { useState, useEffect, useCallback } from 'react';
import MainLayout from '../../layout/MainLayout';
import VerbsData from '../../../data/grammar/A1/3Verbs.json';
import PageTurner from '../../common/PageTurner';
import { ProgressManager } from '../../../utils/ProgressManager';

const VerbsPage = () => {
  console.log('NEW VERBS PAGE LOADED - MainLayout version');
  ...
  useEffect(() => {
    ...
    progressManager.startSession('A1', 'verbs', currentPage + 1);
  ...
  const handleQuestionComplete = (answerData) => {
    console.log('Verbs question completed:', answerData);
    // This gets called when individual questions are answered
  };

  const handleLessonComplete = (completionData) => {
    console.log('Verbs lesson completed:', completionData);
  ...
  const currentPageData = {
    title: `Verbs - Page ${currentPage + 1}`,
    content: pages[currentPage]?.questions || []
  };

  console.log('About to render verbs, currentPageData:', currentPageData);
    ...
    <div>
      ...
      <div className="progress-display" style={{ 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3>Verbs - Page {currentPage + 1} Progress</h3>
            <p>
